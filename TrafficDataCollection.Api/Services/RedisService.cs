    ...
    public class RedisService : IRedisService, IDisposable
    {
        ...
        public async Task<bool> LoadTrafficLightsToRedisAsync()
        {
            try
            {
                ...
                var mappingData = await GetMappingDataAsync();
                _logger.LogInformation($"Found {mappingData.Count} mapping records");
        ...
        // Helper method to get mapping data from mapping_vietmap_lights collection
        private async Task<Dictionary<int, int>> GetMappingDataAsync()
        {
            try
            {
                var mappingCollection = _provider.Connection.GetDatabase().GetCollection<MappingVietmapLight>("mapping_vietmap_lights");
                var mappingRecords = await mappingCollection.Find(_ => true).ToListAsync();

                return mappingRecords.ToDictionary(
                    m => m.VmlLightId,
                    m => m.MapLightId
                );
            }
            ...
            {
                _logger.LogError(ex, "Error retrieving mapping data from mapping_vietmap_lights collection");
